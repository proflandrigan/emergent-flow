...
`colonymind.codegen.traversal.topological_sort`), threads each node's resolved
OUT-port types into the IN ports they feed (via
`colonymind.codegen.wiring.build_wiring_map`), and calls each node's
...
@public_op(name="cm.codegen.infer_graph_types")
