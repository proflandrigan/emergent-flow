"""
colonymind.types.catalog
~~~~~~~~~~~~~~~~~~~~~~~~~~
Built-in type catalog for Colony Mind.
...
``colonymind.nodes`` fires every reference node's self-registration.

The tokens were inventoried from the ``data_type=`` strings used across
``colonymind/nodes/examples/``. The built-in catalog is intentionally **flat** —
...
from colonymind.types.registry import TypeDef, register_type
