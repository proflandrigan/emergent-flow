"""
colonymind.types.registry
~~~~~~~~~~~~~~~~~~~~~~~~~~~
Colony Mind type registry: a nominal type catalog with a subtype relation.
...
from colonymind.ir.common import IRModel

TOP_TYPE = "any"

#: The entry-point group third-party packages publish type tokens under, mirroring
#: the ``colonymind.nodes`` group used for out-of-core node definitions (ADR 0006).
ENTRY_POINT_GROUP = "colonymind.types"
...
class TypeRegistry:
    ...
    def discover(self, *, group: str = ENTRY_POINT_GROUP) -> list[str]:
        ...
        the ``colonymind.types`` group (or an overridden *group*). Each entry point's
