...
emergentflow. This script writes the four contract artifacts the build consumes:
...
All four are byte-identical to what the local server's GET /schema, GET /catalog,
GET /mutation-schema, and GET /session-event-schema serve, because this script and the server
call the SAME emergentflow.server.service functions (ADR 0014 Dec. 5).
...
def export_ui_contracts(generated_dir: pathlib.Path = GENERATED_DIR) -> None:
    """Write ir.schema.json, catalog.json, mutation.schema.json, and
    session_event.schema.json into *generated_dir*."""
