...
A node definition has two natures (see ADR 0005):
  ...
  2. Python behaviour — codegen, executor, type-inference.  These are callables
     and live in :mod:`emergentflow.nodes.contract`; they never serialize.

This module holds nature (1).  ``NodeSpec`` is the complete JSON-able descriptor
a :class:`~emergentflow.nodes.contract.NodeDefinition` exposes via ``to_spec()``;
it is what the registry (Story 4) indexes and the Epic 4 config UI consumes.

Relationship to the IR (``emergentflow.ir``)
------------------------------------------
...
class PortSpec(IRModel):
    ...
    Mirrors ``emergentflow.ir.port.Port`` but carries no ``id`` — ids are minted
    ...
    data_type:
        ...
        and resolved by inference during ``ef.validate``.
    cardinality:
...
class ParamSpec(IRModel):
    ...
    Mirrors ``emergentflow.ir.params.Param`` (``name``, ``type_token``, ``default``)
