"""
emergentflow.nodes.registry
~~~~~~~~~~~~~~~~~~~~~~~~~~
The node-definition registry (Epic 1, Story 4).

The registry indexes :class:`~emergentflow.nodes.contract.NodeDefinition` subclasses
...
from emergentflow.nodes.spec import NodeSpec
...
ENTRY_POINT_GROUP = "emergentflow.nodes"
...
class NodeRegistry:
    """An indexed catalog of :class:`~emergentflow.nodes.contract.NodeDefinition`
    subclasses.
    ...
    def specs(self) -> list[NodeSpec]:
        ...
        -------
        list[NodeSpec]
            One :class:`~emergentflow.nodes.spec.NodeSpec` per registered
    ...
    def discover(self, *, group: str = ENTRY_POINT_GROUP) -> list[str]:
        ...
        point in the ``emergentflow.nodes`` group (or an overridden *group*).
        Each entry point's :meth:`~importlib.metadata.EntryPoint.load` should
        return a :class:`~emergentflow.nodes.contract.NodeDefinition` subclass;
        ...
        group:
            ...
            :data:`ENTRY_POINT_GROUP` (``"emergentflow.nodes"``).
...
def discover(*, group: str = ENTRY_POINT_GROUP) -> list[str]:
    ...
    :class:`~emergentflow.nodes.contract.NodeDefinition` subclasses.  Broken
