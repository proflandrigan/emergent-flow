...
(:mod:`emergentflow.types.registry`) is the authoritative catalog and serializes to
plain JSON (:meth:`~emergentflow.types.registry.TypeRegistry.to_dict`) so the rules
can later ship to the frontend (ADR 0012). The built-in catalog
(:mod:`emergentflow.types.catalog`) registers the core tokens at import time. The
pure rules engine (:mod:`emergentflow.types.compatibility`) decides edge
...
# moment ``emergentflow.types`` is imported — the same import-for-side-effect pattern
