"""
emergentflow.ir.serialize
~~~~~~~~~~~~~~~~~~~~~~~~
Public serialization API for the Emergent Flow graph IR.

This is the first-class, documented surface for turning a :class:`~emergentflow.ir.graph.Graph`
...
1. **Schema-version policy on load** — a serialized graph carries ``schema_version``; this
   ...
   ``CURRENT_SCHEMA_VERSION`` via the Story 9 migration framework (``emergentflow.ir.migrate``)
...
3. **File I/O helpers** — :func:`save_graph` / :func:`load_graph` using the ``.ef.json``
...
ADR refs:
  ...
  - docs/ir-serialization-format.md: JSON-first, ``.ef.json`` at-rest extension.
"""
...
def _reject_if_newer(found: int) -> None:
    ...
    if found > CURRENT_SCHEMA_VERSION:
        raise SchemaVersionError(
            f"graph schema v{found} was written by a newer version of Emergent Flow; "
...
def save_graph(graph: Graph, path: str | os.PathLike[str], *, indent: int | None = 2) -> Path:
    ...
    The ``.ef.json`` extension is the persisted-file convention but is not enforced.
