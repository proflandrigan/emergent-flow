"""
emergentflow.ir.migrate
~~~~~~~~~~~~~~~~~~~~~~
Generic schema-migration framework for the Emergent Flow graph IR.

Migrations operate on a **parsed JSON dict** of a serialized :class:`~emergentflow.ir.graph.Graph`
...
models: it imports nothing from ``emergentflow.ir.serialize`` (which will, in a later story, call
...
older ``schema_version`` up to :data:`~emergentflow.ir.graph.CURRENT_SCHEMA_VERSION`. No concrete
...
def register_migration(from_version: int, step: MigrationStep) -> None:
    ...
    from_version:
        ...
        less than :data:`~emergentflow.ir.graph.CURRENT_SCHEMA_VERSION` — there is no such
...
def migrate_to_current(
    ...
    *,
    ...
    target:
        ...
        :data:`~emergentflow.ir.graph.CURRENT_SCHEMA_VERSION`.
    migrations:
