...
- [x] **NCF (Neural Collaborative Filtering):** a GMF (Generalized Matrix Factorization) + MLP
...
- [x] The model is a `torch.nn.Module` subclass, but the node wraps it inside `FittedRecommender`
...
- [x] **Optional-dependency discipline:** `torch` absent -> typed
...
- [ ] Golden + equivalence via the Story 13 harness (under a `torch`-available CI job).
  **Deferred:** attention-based models (Transformers for sequential recommendation),
  graph neural networks (GNN-based CF), variational autoencoders for CF. **Partially done:**
  golden `ast.parse` + execute-vs-codegen equivalence exists for `ncf`
  (`tests/test_recommend_deep_ncf.py`, `importorskip("torch")`-gated), plus hand-verified
  fit/recommend/exclude_known/determinism tests. Not done: the actual Story 13 parametrized
  cross-registry harness, which hasn't landed yet.
...
- [x] **Two-tower model:** separate user-tower and item-tower encoders (configurable MLP
...
- [x] **Side features:** the two-tower model optionally consumes user-feature and item-feature
  ...
  signals and generalize to cold-start users/items. Implemented via a **dedicated seam**: the
  shared `Fitter` type used by every other algorithm only carries one optional DataFrame
  (`item_features`), so rather than widening that type across ~20 existing fitters, `two_tower`
  gets its own `ef.recommend.fit_two_tower()` wrapper (mirroring `fit()`'s validation/gating) and
  a dedicated `RecommendFitTwoTower` node exposing both `item_features` and `user_features`
  ports; `two_tower` is also registered normally in the algorithm registry with an item-features-
  only fitter shim, so it still works through the generic `recommend.fit` node too.
- [x] At inference time, item-tower embeddings are precomputed; user-tower embeddings are computed
...
- [ ] Golden + equivalence via the Story 13 harness (under a `torch`-available CI job).
  **Deferred:** multi-task learning (engagement + satisfaction objectives), sequence-aware towers
  ...
  for serving. **Partially done:** golden `ast.parse` + execute-vs-codegen equivalence exists for
  `two_tower` via both the generic node (item-features-only path) and the dedicated
  `RecommendFitTwoTower` node (item+user features path)
  (`tests/test_recommend_deep_two_tower.py`, `importorskip("torch")`-gated), plus hand-verified
  fit/recommend/exclude_known/determinism tests and coverage of all three loss variants
  (bce/bpr_loss/softmax_cross_entropy) and the mismatched-embedding-dim validation error. Not
  done: the actual Story 13 parametrized cross-registry harness, which hasn't landed yet.
