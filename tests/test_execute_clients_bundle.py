...
class _EchoHttp:
    """A stand-in HTTP client whose identity we can assert was threaded."""

    marker = "HTTP_CLIENT_REACHED"


@register
class _HttpEchoNode(NodeDefinition):
    type = "_test.http_echo"
    family = "_test"
    label = "HTTP Echo (test-only)"
...
def test_http_node_receives_http_seam():
    results = execute(_http_node_graph(), clients=Clients(http=_EchoHttp()))
    assert results["n1"]["out"] == "HTTP_CLIENT_REACHED"
