...
ef_texttools.nodes
~~~~~~~~~~~~~~~~~~
Out-of-core node plugin for Emergent Flow (Epic 1, Story 4).
...
the entry point declared in ``pyproject.toml`` under the ``emergentflow.nodes``
...
class ReverseText(NodeDefinition):
    ...
    def codegen(self, node: Node, ctx: CodegenContext) -> CodeFragment:
        return CodeFragment(
            imports=["from ef_texttools.nodes import reverse_text"],
