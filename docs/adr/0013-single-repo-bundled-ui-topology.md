...
(#11) commit the product to **three separate repositories** — `emergent-flow` (this SDK),
`emergent-flow-canvas` (React/TS frontend), and `emergent-flow-server` (FastAPI/Celery backend) —
...
`import emergentflow`; the only things crossing the line are the IR JSON Schema, the
...
- **Distribution.** `pip install emergentflow` → `emergentflow serve` is dramatically simpler to
...
distributable Python package, `emergentflow`, with the canvas UI bundled in — the JupyterLab
...
1. **One repo, one package.** This repository holds the SDK, the canvas UI, and (from Phase 2)
   a thin local server. We do **not** create `emergent-flow-canvas` or `emergent-flow-server` as
   separate repositories. Internal layout:

   ```
   emergent-flow/
     emergentflow/          # Python SDK + IR + codegen (unchanged)
       server/            # thin FastAPI app (Phase 2): serves the UI, calls ef.*
       _static/           # build artifact: compiled UI assets bundled into the wheel
     ui/                  # TypeScript/React canvas — own package.json, Vite, Vitest
     pyproject.toml       # build hook: `vite build ui/` → emergentflow/_static/
   ```

2. **Bundled distribution.** The package build compiles `ui/` into `emergentflow/_static/` and
   includes it in the wheel. `pip install emergentflow` plus a launch command
   (`emergentflow serve`, alias `ef lab`) boots a local server that serves the canvas. The SDK
...
3. **The coupling invariant is retained verbatim.** `ui/` **must not** import or bundle
   Python. The only artifacts crossing the `ui/ ↔ emergentflow/` boundary are the three §A5
   ...
   that calls `ef.*`. This keeps [ADR 0001](0001-graph-is-single-source-of-truth.md) /
...
4. **Replace the repo wall with a CI wall.** Because the physical repo boundary no longer
   ...
   dependency path into `emergentflow` internals, and (b) assert that only the three named
   ...
   (`uv`/`ruff`/`mypy`/`pytest` for `emergentflow/`; `npm`/Vite/Vitest for `ui/`), both run in
   one CI workflow.

5. **Phase-1 simplification (consequence we will exploit).** Because a thin local server can
   call the real `ef.validate` / `ef.compile_to_code` over localhost, the canvas does **not**
...
- ~~The concrete build-hook implementation and wheel-bundling test.~~ **Done (Epic 4 Story 4d):**
  ...
  `backend-path = ["."]`) runs `vite build ui/` → `emergentflow/_static/` on a real wheel build
...
- **Update (2026-06-23):** a minimal local server now exists — `emergentflow/server/` with a
  `emergentflow serve` / `ef lab` entry point that calls `ef.compile_to_code` / `ef.execute` /
  `ef.validate` **in-process**. To keep the bundled install lean and zero-dependency for this
  ...
  `emergentflow/_static/` when present (Story 4) and exposes incremental `run_to` / `run_node`
  execution (Story 6).
