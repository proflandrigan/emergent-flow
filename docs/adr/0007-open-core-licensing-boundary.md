...
1. **Licensing:** The `emergentflow` Python SDK will be licensed under the **Apache License, Version 2.0 (Apache-2.0)**. This permissive license provides an explicit patent grant and ensures the SDK is freely embeddable.
...
3. **SDK Inventory:** The open-source SDK includes:
   - IR schema and models (`emergentflow/ir`).
   - The node-definition contract and registry (`emergentflow/nodes`).
