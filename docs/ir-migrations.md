...
describes the migration framework (`emergentflow/ir/migrate.py`) and the policy that governs it.

## Model

- Every serialized graph embeds a `schema_version` (an integer). The version this build
  understands is `emergentflow.ir.CURRENT_SCHEMA_VERSION`.
...
All are re-exported from `emergentflow.ir`.

## On load

`deserialize_graph` (in `emergentflow.ir.serialize`) applies the policy:
...
```python
from emergentflow.ir.migrate import register_migration
